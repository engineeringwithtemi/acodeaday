...
import { Calendar, Clock, Code, AlertCircle, Layers, ChevronDown, ChevronRight, CheckCircle2, Circle, Download } from 'lucide-react'
import { useTodayProblems, usePatterns, useImportHistory } from '@/hooks'
import { ImportModal } from '@/components/ImportModal'
import { ActiveImportTracker, ImportHistoryList } from '@/components/ImportStatusPanel'
...
function Dashboard() {
  ...
  // Recent imports (non-active, for history display)
  const recentImports = (importHistory ?? []).filter(
    (j) => j.id !== activeImportId
  )
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-cyan-500/10 border border-cyan-500/30 hover:bg-cyan-500/20 text-cyan-400 font-semibold rounded-lg transition-colors text-sm"
            >
              <Download size={16} />
              Import Problems
            </button>
        ...
        <ImportModal
          ...
          onImportStarted={(id) => setActiveImportId(id)}
        />
        ...
        {recentImports.length > 0 && (
          <div className="mb-8">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-2xl font-bold text-white">Recent Imports</h2>
              <span className="px-3 py-1 bg-cyan-500/20 text-cyan-400 rounded-full text-sm font-semibold">
                {recentImports.length}
              </span>
            </div>
            <ImportHistoryList imports={recentImports} maxItems={5} />
          </div>
        )}
